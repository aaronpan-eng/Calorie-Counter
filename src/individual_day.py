...
def calorieEstimator(image_path, model_path):
    ...
    for result in results.boxes.data.tolist():
        ...
        counter = 0
        if results.names(int[class_id]) != 'screen':
            if (score > threshold) and (counter == 0):
...
def add_food(current_date, day_window):
    ...
    food_model_path = os.path.join(current_directory, 'runs', 'detect', 'train4', 'weights', 'last.pt')
    scale_model_path = os.path.join(current_directory, 'runs', 'detect', 'train8', 'weights', 'last.pt')
